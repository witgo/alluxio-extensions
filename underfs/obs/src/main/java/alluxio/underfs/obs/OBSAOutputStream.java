 ...
 */
@NotThreadSafe
public final class OBSAOutputStream extends OutputStream {
  private static final Logger LOG = LoggerFactory.getLogger(OBSAOutputStream.class);
  ...
  /**
   * Creates a name instance of {@link OBSAOutputStream}.
   *
   * @param bucketName the name of the bucket
   * @param key        the key of the file
   * @param client     the OBS client
   */
  public OBSAOutputStream(String bucketName, String key, ObsClient client) throws IOException {
    ...
    mKey = key;
    mObsClient = client;
    mBuf = new ByteArrayOutputStream(512);
    try {
      mHash = MessageDigest.getInstance("MD5");
      mLocalOutputStream = new DigestOutputStream(mBuf, mHash);
    } catch (NoSuchAlgorithmException e) {
      LOG.warn("Algorithm not available for MD5 hash.", e);
      mHash = null;
      mLocalOutputStream = mBuf;
    }
  }
  ...
  public void flush() throws IOException {
    try {
      ...
      ByteArrayInputStream in = new ByteArrayInputStream(mBuf.toByteArray());
  ...
  public void close() throws IOException {
    if (mClosed.getAndSet(true)) {
      LOG.warn("OBSAOutputStream is already closed");
      return;
    }
    flush();
  }
